...
switch lower(method),
 ...
 case 'difference',
  ...
  if (size(V,3) >= 2*ord+1),
    H(2,3,:,m2,:) = 1/dx * squeeze(sum(V(ind).*aa1(:,:,m2,:)));
    ...
    H(2,3,:,:,m3) = 1/dz * squeeze(sum( ...
        reshape(H(2,3,ind),size(ind)).*aa1(:,:,:,m3)));
    H(2,3,:,:,[1:ord end-ord+1:end]) = NaN;
    H(3,2,:,:,:) = H(2,3,:,:,:);

    H(1,3,:,:,m3) = 1/dz * squeeze(sum(V(ind).*aa1(:,:,:,m3)));
    ind = sub2ind(size(V), ii(:,m1,:,:)+off(:,m1,:,:),...
                  jj(:,m1,:,:), kk(:,m1,:,:));
    H(1,3,m1,:,:) = 1/dy * squeeze(sum( ...
        reshape(H(1,3,ind),size(ind)).*aa1(:,m1,:,:)));
